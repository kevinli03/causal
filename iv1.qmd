...
The OLS estimate for $\tau_{LATE}$ is our causal estimate.
...
We can test relevance by running a regression of $D$ on $Z$ (which is the 1st stage of 2SLS), and see if $\beta$ is significant:
...
-   Weak instruments can have very biased $\tau_\text{LATE}$ estimates, especially in small samples.
