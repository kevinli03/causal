...
$$
Y_i = \hat\alpha + D_i\hat\tau_\text{ATE} + \mathbf X_i^\top \hat{\pmb\beta} + \hat\varepsilon_i
$$
...
When we hold confounders constant, treatment is exogeneous. Thus, $\tau_\text{ATE} | \mathbf X$ (effect given confounders $\mathbf X$) is identified. To get the overall $\tau_\text{ATE}$, we weight all of $\tau_\text{ATE} | \mathbf X$ for each value of $\mathbf X$ together:

$$
\tau_\text{ATE} = \underbrace{\int (\tau_\text{ATE}| \mathbf X) \cdot d \ Pr(\mathbf X)}_{\text{weighted average of }\tau|\mathbf X}
$$

Issue: Angrist (1998) show that OLS estimate of $\tau$ equals:

$$
\tau_\text{OLS} = \int (\tau_\text{ATE}|\mathbf X) \cdot d \ \underbrace{\frac{Var(D_i|\mathbf X_i)Pr(\mathbf X_i)}{\sum_{\mathbf X^c} Var(D_i|\mathbf X^c) Pr(\mathbf X^c) }}_\text{weight}
$$

The weight of $\tau_\text{OLS}$ is clearly not equal to $\tau_\text{ATE}$. Thus, if there is any treatment heterogeneity, OLS will not estimate the ATE.
...
$$
\log\left( \frac{\pi}{1-\pi}\right) = \beta_0 + \beta_1X_{i1} + \beta_2 X_{i2} + \dots + \beta_p X_{ip}
$$
...
$$
X_i = \beta_0 + \beta_1 D_i + \varepsilon_i
$$
