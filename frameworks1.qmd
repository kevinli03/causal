...
We generally assume that treatment $D$ is binary:

$$
D_i =\begin{cases}
1 & \text{if individual }i \text{ gets the treatment (treated)} \\
1 & \text{if individual }i \text{ does not get the treatment (untreated)} \\
\end{cases}
$$

Note: $i$ refers to an individual within our data. For example, we might have Ben, Ava, Anna, etc. in our study. $i$ is any specific individual.

## Example

We might be interested in how receiving a scholarship affects academic performance (let us say GPA)
...
1 & \text{if individual }i \text{ does not get scholarship (untreated)} \\
\end{cases}
$$
...
Our goal for causal inference is thus to make our treatment $D$ exogenous (or achieve exogeneity).

## Randomisation and Causation

What is the simplest way to make treatment $D$ exogenous?

-   Well obviously - we run a controlled experiment, and we randomly assign individuals to treated and untreated groups!

These are called **Randomised Experiments** - they are the **gold standard** (best method) for causal inference.

**Issue**: It is hard to run randomised experiments for several reasons:

1.  Expensive: even big universities often do not have the budget.
2.  Impossible: Lets say your treatment is democracy. You would have to randomly assign countries to be democracies or dictatorships. Well you can't really tell Canada - Hey, I randomly decided you are going to be a dictatorship.

Thus, this course covers many methods to make treatment exogenous (random) without randomised experiments.
