...
## Our Issue in Potential Outcomes
...
$$
\overline Y^{(G=1)}_{t=0} + (\text{trend}) = \overline Y^{(G=1)}_{t=1} + \left(\overline Y^{(G=0)}_{t=1} - \overline Y^{(G=0)}_{t=0}\right)
$$

The estimated causal effect is thus:

$$
\tau_\text{ATT} = \overline Y^{(G=1)}_{t=0} - \underbrace{\left [\overline Y^{(G=1)}_{t=1} + \left(\overline Y^{(G=0)}_{t=1} - \overline Y^{(G=0)}_{t=0}\right) \right]}_\text{estimated counterfactual}
$$
...
$$
\tau_\text{ATT} = \underbrace{\left(\overline Y^{(G=1)}_{t=0} - \overline Y^{(G=1)}_{t=1}\right)}_{\Delta \text{ for } G=1} - \underbrace{\left(\overline Y^{(G=0)}_{t=1} - \overline Y^{(G=0)}_{t=0}\right)}_{\Delta \text{ for } G=0} 
$$
...
The total causal effect is the difference of these two differences.

## Parallel Trends Assumption

The causal estimate $\tau_\text{ATT}$ is only accurate if the parallel trends assumption is met.
...
ggplot() +
  ...
  scale_color_manual(
    ...
    values = c("col1" = "red",
               ...
               "col4" = "purple",
               "col5" = "black"),
    labels = c("Pennsylvania", "New Jersey", "New Jersey (Parallel Trends)", "New Jersey (Real Counterfactual)", "Treatment Effect")
  ) +
  theme_bw() +
  scale_x_continuous(breaks = c(0, 1), labels = c("Mar 1992","Dec 1992")) +
  xlab(NULL) + ylab("Employment Rate") +
