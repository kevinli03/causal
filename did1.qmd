...
$$
\hat\tau_\text{ATT} = \underbrace{\left(\overline Y^{(G=1)}_{t=0} - \overline Y^{(G=1)}_{t=-1}\right)}_\text{Diff. for treated group} - \underbrace{\left(\overline Y^{(G=0)}_{t=0} - \overline Y^{(G=0)}_{t=-1}\right)}_\text{Diff. for control group} 
$$
...
The estimate of $\hat\tau_\text{ATT}$ is our DiD estimate of the causal effect. It will be equal to the estimate from the equation on the last side.

-   Standard errors should be clustered at the group $G_i$ level.

## In R

Regression estimator for classical DiD:
...
-   Where group is a binary variable: treatment = 1, control = 0

-   Where time is a binary variable: post-treatment = 1, pre-treatment = 0.

-   Coefficient group::time in the output is the estimated ATT.
